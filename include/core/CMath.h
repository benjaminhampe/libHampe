...
namespace core
{
	...
	template<class T>
	core::vector2d<T> transformPoint2d(
		const core::vector2d<f32>& v_src,
		const core::rect<f32>& r_src,
		const core::rect<T>& r_dst )
	{
		core::vector2d<f32> v;
		...
		return core::vector2d<T>(v.X,v.Y);
	};
...
public:
	...
	template<class T> static T WrapAngle(T x)
	{
		while (x < (T)0)
			x += (T)360;
		while (x > (T)360 || core::equals( x, (T)360) )
			x -= (T)360;
	...
	template<class T> static T WrapAngle180(T x)
	{
		...
		while (x > (T)180 || core::equals( x, (T)180))
		{
			x -= (T)360;
		}
	...
	template <class T>  core::stringc analyzeData( const array<T>& data, bool bPrintDataToo = false ) const
	{
		core::stringc txt("Statistical Analysis of LinearData (array) with ");
